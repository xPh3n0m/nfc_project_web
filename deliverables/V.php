...
$sql = 'select m.country, sum(case m.medal when \'Gold medal\' then 1 else 0 end) as Gold_medal, sum(case m.medal when \'Silver medal\' then 1 else 0 end) as Silver_medal, sum(case m.medal when \'Bronze medal\' then 1 else 0 end) as Bronze_medal, count(*) as Total
